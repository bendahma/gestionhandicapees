            ...
            <div class="card-body">
                <form method="POST" action="{{isset($rappel) ? route('rappel.update', $rappel->id) : route('rappel.store') }}">
                    ...
                    @if(isset($hand))
                        @method('PATCH')
                    @endif
                    <h4 class="text-danger font-weight-bold">Les informations Du Handicapée</h3>
                    <hr>
                    <div class="row mt-1">
                        <div class="col">
                            <div class="form-group">
                                <label for="name" class="font-weight-bold">Nom & Prenom</label>
                                <input type="text" class="form-control" required id="name" name="nameFr" value="{{isset($hand) ? $hand->nameFr : ''}}">
                              </div>
                         </div>
                        <div class="col">
                            <div class="form-group">
                                <label for="" class="font-weight-bold">Date de Naissance</label>
                                <input type="date" class="form-control" id="" name="dob" value="{{ isset($hand) ? $hand->dob : '' }}">
                            </div>
                        </div>
                        <div class="col">
                           <div class="form-group">
